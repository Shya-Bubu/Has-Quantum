...
function renderLecture() {
    ...
    if (!window.LECTURES) return;
    
    const lecture = window.LECTURES.find(l => l.id === id);

    if (!lecture) {
        if (!window.lecturesLoaded) {
            // Keep waiting for list.txt
            return;
        }
    ...
    // Render notes/errata section (above video)
    const notesSection = document.getElementById('notes-section');
    if (notesSection) {
        const notes = (window.LECTURE_NOTES && window.LECTURE_NOTES[lecture.id]) || [];
        if (notes.length > 0) {
            const noteTypeConfig = {
                correction: { icon: '⚠️', label: 'Correction', className: 'note-correction' },
                info:       { icon: 'ℹ️', label: 'Note',       className: 'note-info' },
                tip:        { icon: '💡', label: 'Tip',        className: 'note-tip' }
            };

            const notesHtml = notes.map(note => {
                const config = noteTypeConfig[note.type] || noteTypeConfig.info;
                return `
                    <div class="note-item ${config.className}">
                        <div class="note-icon">${config.icon}</div>
                        <div class="note-body">
                            <span class="note-label">${config.label}</span>
